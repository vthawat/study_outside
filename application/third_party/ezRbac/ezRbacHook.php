...
class EzRbacHook
{
    ...
    {
        $this->CI->load->model('userinfo');
        $access_token=get_cookie('access_token');   
        $userinfo=$this->CI->userinfo->getOAuthUser($access_token);
        if($this->CI->userinfo->status_code!=200)  // valid access_token
           redirect(base_url('psuoauth2')); // invalid access_token handrer
    }
