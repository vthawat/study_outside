  ...
  <form action="" method="post">
    ...
    <?php  if(!empty($stop_time))if($i!=0)if($this->study_trip->isTimeBreak($keep_end_time[$i-1],$start_time)):?>
        ...
        <tr>
            ...
            <td><input type="hidden" class="cut-schedule" value="<?=$rout->start_place_id?>"> จาก<span class="schedule-arrive-place-day<?=$days?>"><?=$rout->start_location?> <?=$start_location_details?></span> <i class="fa fa-fw fa-angle-double-right"></i>ถึง<span class="schedule-depart-place-day<?=$days?>"><?=$rout->end_location?> <?=$end_location_details?></span><input type="hidden" class="end_place_id" value="<?=$rout->end_place_id?>"></td>
        </tr>
        <?php  if(!empty($stop_time))if($this->study_trip->isTimeBreak($start_time,$end_time)):?>
            ...
            <?php  $end_time=0;$days++?>
                ...
                <ul class="list-group place-listed">
                 ...
                 <div class="text-right"><a class="btn icon-btn btn-warning" href="<?=base_url('staff/place_rest/new');?>"><span class="btn-glyphicon fa fa-bed img-circle text-warning"></span>เพิ่มสถานที่พักค้างคืน</a></div>
                 </td>
                </tr>
                <tr>
                 <td></td>
